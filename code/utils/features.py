...
def extract_features(imgs, feature_fns, verbose=False):
    ...
    for feature_fn in feature_fns:
        # print(f'imgs[0].squeeze():{imgs[0].squeeze().shape}')
...
def hog_feature(im):
    ...
    # print(f"orientation_histogram.shape: {orientation_histogram.shape}")
    for i in range(orientations):
        ...
        orientation_histogram[:, :, i] = uniform_filter(
            temp_mag, size=(cx, cy))[round(cx/2)::cx, round(cy/2)::cy].T
